...
- 在 `docs/ui_design_spec.md` 新增第 15 章「吸附式窗口面板方案（待架构师审核）」，记录把左栏、右栏、底栏从 `SidePanel` / `TopBottomPanel` 改为 egui 吸附窗口的候选架构：动机、默认布局、吸附规则、持久化、对现有代码影响、工作量与风险。同时更新第 9 章「暂不做」中 docking 条目的状态。
