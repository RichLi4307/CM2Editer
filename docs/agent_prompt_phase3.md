...
**状态：Phase 3 已完成（2026-07-07），三轮修复已全部落地（2026-07-08）。当前项目进入 Phase 5 新功能开发。**

**前置检查：**

- [x] Phase 1（数据层）已完成：graph 数据结构、api 注册表、验证器
- [x] Phase 2（序列化与代码生成）已完成：JSON 双向转换、.code 生成
- [x] Phase 3（UI 层）已完成：egui 四栏布局、节点/连线渲染、交互、面板、工具栏
- [x] Phase 3 修复已完成：框选、多选拖拽、连线选中/删除、复制/粘贴、右键菜单、快捷键、文件对话框、搜索窗口、视口裁剪、损坏 JSON 处理、空画布体验等

> ⚠️ 开始 Phase 5 前务必先读 `docs/问题清单.md` 与 `docs/TODO.md`，明确 DataFlow 重构、参数类型重构、命名空间管理等高复杂度任务。
> UX 债务：拖线实时环检测反馈、空画布启动体验、搜索窗口关闭渠道 — 已解决。
> 下一阶段 UX 重点：端口吸附环、DataFlow 数据端口、参数下拉表、命名空间选择窗口。
...
>
> **Phase 5 注意**：DataFlow 重构将扩展节点参数编辑方式（下拉选择数据源、边框 Data 端口、虚线数据连线），并可能影响 `ParamValue` 序列化格式与 `properties.rs` 面板实现。实现前请结合 `docs/问题清单.md` 与 `docs/TODO.md` 中的高复杂度任务清单进行设计。
