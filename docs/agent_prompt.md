...
- 编辑器中间格式：JSON（见 `docs/json_schema.md`）
...
编辑器保存的文件格式（完整字段定义见 `docs/json_schema.md`）：
...
- 矩形卡片，圆角 8px
- 标题栏高度 32px，颜色按分类区分（见 `docs/node_types.md` 颜色编码表）
...
每个节点类型必须有以下元数据。完整节点清单见 `docs/node_types.md`。
...
- 示例任务：`docs/examples/new npc type/main.code` 与 `meta.json`

> 备注：本项目中 `ui_spec.md` 与 `documentation_zh.html` 尚未提供，UI 细节以本文件第五节和 `docs/node_types.md` 颜色编码表为准。
