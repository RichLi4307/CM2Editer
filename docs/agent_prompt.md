...
| 类别 | 定义 | 规则 |
|------|------|------|
| **A 类**（自定义代码生成） | 有显式 `match` 臂的节点（If/While/Goto/CallFunction 等） | 新增 Data 输出端口**必须手动写** `var_{id}_{port} = ...`；端口值需被引用时**还必须在 `evaluate_data_output` 加分支** |
| **B 类**（通用代码生成） | 走 `generate_node_call` 的节点（大多数 Flow 节点） | 新增 Data 输出端口**自动生成**，无需改代码生成器 |
| **C 类**（纯 Data 节点） | 无 Flow 端口的节点（Boolean/GetState*/CompareNumbers 等） | 新增需在 `evaluate_data_output` 加分支 |
