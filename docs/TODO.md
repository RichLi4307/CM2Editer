...
- [ ] **P0.8 多分支 If 节点（elseif 单节点化）**
  ...
  - 设计方向：
    - 保留现有 `If` 节点的 `out_true` / `out_false` 不变（避免破坏旧图），新增 `IfMulti` / `MultiBranch` 节点或升级 `If` 为动态端口模式
    - 推荐方案：升级 `If` 节点本身，使其 `out_true` 固定为第一个分支，后续分支通过动态端口添加；向后兼容通过旧图反序列化时自动迁移（旧 `out_true`/`out_false` 转换为 2 分支结构）
