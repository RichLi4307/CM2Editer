...
| 1.3 | 点击"新建工程" | 弹出新建对话框，填写名称后创建含 Start 节点的空图 | JM | |
...
| # | 测试项 | 预期 | 结果 | 备注 |
|---|--------|------|------|------|
| 2.1 | 工程标签：节点库可搜索 | 输入文字过滤节点列表 | JM | |
| 2.2 | 工程标签：展开节点分类 | CollapsingHeader 展开/折叠 | JM | |
| 2.3 | 工程标签：点击节点创建 | 画布上出现新节点 | JM | 现在已改成拖出标签放置节点，成功,但拖出的虚影宽度太窄，单个单词多次换行 |
| 2.4 | 工程标签：下方项目文件树 | 显示 meta.json 和 .code 文件 | JM | |
| 2.5 | 命名空间标签：命名空间按钮可点击 | 弹出命名空间浏览窗口 | JM | |
| 2.6 | 命名空间标签：cosplay 有二级分类 | 展开 cosplay → 显示头部/上装/下装/生殖等子分类 | JM | 美术待优化 |
| 2.7 | 命名空间标签：点击卡片复制 key | 复制到剪贴板 + 状态栏提示 | JM | |
| 2.8 | 坐标标签：场景分组 | 按 stage 分组显示 | JM | 没有删除按钮 |
| 2.9 | 坐标标签：展开场景 | CollapsingHeader 展开/折叠，内部显示坐标名和 x/y/z | JM | |
| 2.10 | 坐标/命名空间 Add 按钮 | 内联表单可添加条目，添加后即时刷新 | NT | 无删除按钮，保险起见未测试 |
...
| # | 测试项 | 预期 | 结果 | 备注 |
|---|--------|------|------|------|
| 5.1.1 | 拖入 Boolean 节点 | 出现在画布上 | JM | |
| 5.1.2 | 参数 value | 下拉框显示 true / false | JM | |
| 5.1.3 | 切换 value | 选中后更新 | JM | |
| 5.1.4 | out_value 端口类型 | Boolean | JM | |
