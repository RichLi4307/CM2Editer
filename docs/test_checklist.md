...
| 1.4 | 窗口大小 | 占屏幕大部分区域（≈75%） | JM | 窗口默认最大化，无闪烁 |
...
| 2.1.5 | 拖出节点放置画布 | 画布上出现新节点 | JM | 节点库按钮最小宽度 120px，虚影单行无换行 |
...
| 2.3.4 | Add 按钮 | 内联表单可添加条目，添加后即时刷新 | JM | 每条条目右侧新增删除按钮，需二次确认 |

### 2.4 坐标标签

| # | 测试项 | 预期 | 结果 | 备注 |
|---|--------|------|------|------|
| 2.4.1 | 场景分组 | 按 stage 分组显示 | JM | 每条条目右侧新增删除按钮，需二次确认 |
| 2.4.2 | 展开场景 | CollapsingHeader 展开/折叠，内部显示坐标名和 x/y/z | JM | |
| 2.4.3 | Add 按钮 | 内联表单可添加条目 | JM | 删除按钮二次确认 |
