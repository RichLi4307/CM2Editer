...
| 1.3 | 点击"新建工程" | 弹出新建对话框，填写名称后创建工程；画布含默认 ThreadContainer（main 标签入口） | DN | 我强调过很多回不需要我们自动新建线程，用户自己忘记创建线程是用户自己的事。但是我们的线程如果无法删，一些无需线程也能运行的模组就难以创建了 |
...
| 2.1.3 | 搜索框按节点名过滤 | 输入文字过滤节点列表 | DN | 不支持大写字母 |
...
| 2.1.7 | 节点悬停/属性面板描述 | 属性面板显示 1–2 句中文详细描述 | DN | 工程栏/画布悬停无描述，属性面板有描述 |
...
| 2.2.3 | 文件树独立滚动 | 上下文过长时文件树自己出现滚动条，底部按钮不被挤出 | DN | 底部控件仍可超出屏幕外，且控件下降速度比文件树下降速度慢 |
...
| 3.6 | Space 打开搜索 | 搜索窗口弹出，输入节点名过滤 | JM | 与左栏相同，无法使用大写字母 |
