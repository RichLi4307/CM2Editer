...
**v0.2.1** —— 自 0.1.1 以来新增 52 个节点类型（143→159），重构了代码生成器以正确输出 CM2 加载器兼容的任务文件。现已支持完整的状态机逻辑（Thread/Listener/Goto）、条件系统（CreateCondition→CheckCondition 管道）、坐标预设与命名空间管理。底部面板三列可拖拽布局，文本框持久缓冲区无吞字，全局热键门控确保画布/属性面板互不干扰。110 项测试通过。
